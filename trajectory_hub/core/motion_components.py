...
class MacroTrajectory(MotionComponent, MovementModeMixin):
    ...
    def __init__(self):
        ...
        self.current_update_frame = -1  # Para evitar múltiples actualizaciones de fase por frame
    ...
    def update(self, state: MotionState, current_time: float, dt: float) -> MotionState:
        ...
        # Frame tracking para evitar múltiples actualizaciones de fase por frame
        update_frame_id = int(current_time * 1000)  # Aproximar frame ID
        if self.current_update_frame != update_frame_id:
            # Nuevo frame, actualizar fase solo una vez
            self.current_update_frame = update_frame_id
        ...
        if self.trajectory_func:
            ...
            if isinstance(new_position, np.ndarray) and new_position.shape == (3,):
                # NO actualizar last_position aquí - se hace en calculate_delta()
                state.position = new_position
