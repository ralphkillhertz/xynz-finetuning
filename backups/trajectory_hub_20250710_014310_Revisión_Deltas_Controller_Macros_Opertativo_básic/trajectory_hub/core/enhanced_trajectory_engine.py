...
class EnhancedTrajectoryEngine:
    ...
    def create_macro(
        ...
        **kwargs
        ...
        # Enviar posiciones a Spat inmediatamente
        if hasattr(self, 'osc_bridge') and self.osc_bridge:
            try:
                # Obtener posiciones del macro recién creado
                macro = self._macros[macro_id]
                positions = {}
                
                # Si el macro tiene sources con posiciones
                if hasattr(macro, 'sources'):
                    for i, source in enumerate(macro.sources):
                        if hasattr(source, 'x'):
                            positions[i] = (float(source.x), float(source.y), float(source.z))
                        elif hasattr(source, 'position'):
                            pos = source.position
...
# Enviar posiciones OSC al final de cada update
        ...
        if self.osc_bridge and hasattr(self, '_positions') and self._positions is not None:
            ...
            if hasattr(self, '_active_sources') and self._active_sources:
                ...
                n_sources = min(len(self._positions), self.config.get('n_sources', 16))
        ...
        try:
            if self.osc_bridge and hasattr(self, '_positions') and hasattr(self, '_active_sources'):
                positions = {}
                for sid in list(self._active_sources)[:16]:  # Max 16 sources
                    if sid < len(self._positions):
