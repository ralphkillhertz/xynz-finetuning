...
class ManualMacroRotation(MotionComponent):
    ...
    def calculate_delta(self, state: 'MotionState', current_time: float, dt: float) -> Optional['MotionDelta']:
        """Calcula el cambio necesario para la rotación manual usando matrices de rotación 3D"""
        if not self.enabled:
            return None
        ...
        # Interpolar ángulos actuales hacia los objetivos
        # interpolation_speed es ahora en radianes/segundo (0.01 a 2.0 rad/s típicamente)
        # Para una rotación completa (2π) a velocidad 1.0 tomaría ~6.3 segundos
        factor = min(self.interpolation_speed * dt, 1.0)  # Limitar a 1.0 para evitar overshooting
        ...
        # Calcular matriz de rotación 3D completa
