# === diagnose_engine_operation.py ===
# 🔧 Fix: Entender cómo funciona realmente el engine
# ⚡ Buscar el flujo de actualización

import os

print("🔍 Analizando cómo funciona EnhancedTrajectoryEngine...")
...
# Buscar la clase
class_pos = content.find("class EnhancedTrajectoryEngine")
if class_pos != -1:
    # Ver los primeros métodos y atributos
    lines = content[class_pos:class_pos+2000].split('\n')
    print("\n📋 Primeras líneas de EnhancedTrajectoryEngine:")
    for i, line in enumerate(lines[:30]):
        if line.strip():
            print(f"  {line[:80]}")

# Buscar referencias a fps o frame
print("\n🔍 Referencias a fps/frame:")
for i, line in enumerate(content.split('\n')):
    if any(word in line.lower() for word in ['fps', 'frame', 'rate']):
        print(f"  L{i+1}: {line.strip()[:80]}")

# Buscar dónde se usan _positions
print("\n🔍 Dónde se modifican _positions:")
lines = content.split('\n')
for i, line in enumerate(lines):
    if '_positions[' in line and '=' in line:
        # Mostrar contexto
        print(f"\n  L{i+1}: {line.strip()}")
        # Ver método contenedor
        for j in range(max(0, i-10), i):
            if 'def ' in lines[j]:
                print(f"    En método: {lines[j].strip()}")
                break

# Crear el método update si no existe
create_code = '''# === add_engine_update_method.py ===
import os
import shutil
...
with open("add_engine_update_method.py", "w") as f:
    f.write(create_code)

print("\n✅ Script creado: add_engine_update_method.py")
print("\n📝 Parece que el engine NO tiene un método update()")
print("   Necesitamos crearlo para automatizar el procesamiento de deltas")