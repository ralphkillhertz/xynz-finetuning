...
class InteractiveController:
    ...
    def _show_main_menu(self):
        ...
        options = [
            ("1", "🎯 Crear Macro/Grupo"),
            ("2", "🌀 Sistema de Movimiento (Deltas)"),
            ("3", "🎵 Modulador 3D"),
    ...
    def _show_delta_menu(self):
        """Menú específico del sistema de deltas"""
        options = [
            ("== COMPONENTES DELTA ==", ""),
            ...
            ("4", "MacroRotation - Rotación algorítmica de grupo"),
            ("5", "ManualMacroRotation - Rotación manual de grupo"),
            ("6", "IndividualRotation - Rotación algorítmica individual"),
            ("7", "ManualIndividualRotation - Rotación manual individual"),
            ("-", ""),
            ("b", "Volver"),
            ("q", "Salir")
        ]
    ...
    def _process_delta_choice(self, choice: str):
        """Procesa opciones del menú delta"""
        # Redirige a las funciones específicas del menú movement
        ...
        mapping = {
            ...
            "4": "4",  # MacroRotation
            "5": "4",  # ManualMacroRotation (mismo menú)
            "6": "5",  # IndividualRotation
            "7": "5"   # ManualIndividualRotation (mismo menú)
        }
    ...
    def _create_macro_rotation_command(self) -> Optional[SemanticCommand]:
        ...
        if rotation_type == 0:  # Algorítmica
            ...
            if config_choice == 0:  # Presets
                presets = [
                    ("🌀 Rotación suave", {"speed_x": 0.5, "speed_y": 0.3, "speed_z": 0.2}),
                    ("🌊 Oleaje flotante", {"speed_x": 0.0, "speed_y": 0.8, "speed_z": 0.4}),
                    ("🌪️ Vórtice dinámico", {"speed_x": 1.5, "speed_y": 1.5, "speed_z": 0.0}),
                    ("🎭 Péndulo teatral", {"speed_x": 0.0, "speed_y": 1.0, "speed_z": 0.0}),
                    ("🎪 Carrusel mágico", {"speed_x": 0.0, "speed_y": 0.0, "speed_z": 1.2}),
                    ("🎨 Espiral artística", {"speed_x": 0.7, "speed_y": 0.5, "speed_z": 0.9}),
                    ("🎯 Precisión robótica", {"speed_x": 0.2, "speed_y": 0.2, "speed_z": 0.2}),
                    ("🌌 Órbita galáctica", {"speed_x": 0.4, "speed_y": 0.6, "speed_z": 0.3})
                ]
            ...
            elif config_choice == 2:  # Simplificada
                speed = self.ui.get_numeric_input("Velocidad de rotación (0.1-2.0): ", 0.1, 2.0) or 1.0
                depth = self.ui.get_numeric_input("Profundidad/Complejidad (0-100%): ", 0, 100) or 50
                
                # Convertir a velocidades individuales basadas en profundidad
                depth_factor = depth / 100.0
                speed_x = speed * 0.7 * depth_factor
                speed_y = speed * 0.5 * (1.0 - depth_factor * 0.5)
                speed_z = speed * 0.3 * depth_factor
                
            else:  # Función personalizada
                # Importar la biblioteca de funciones
                from ..core.custom_motion_functions import motion_library
                
                # Opciones de función personalizada
                custom_options = [
                    "📚 Seleccionar de biblioteca",
                    "✍️ Escribir expresión nueva",
                    "🌟 Ejemplos de expresiones"
                ]
                
                custom_choice = self.ui.get_choice_from_list(
                    custom_options,
                ...
                if custom_choice == 0:  # Biblioteca
                    functions = motion_library.list_functions()
                    func_names = list(functions.keys())
                    func_descriptions = [f"{name}: {desc}" for name, desc in functions.items()]
                    
                    func_idx = self.ui.get_choice_from_list(
                        func_descriptions,
                        "Seleccione función de movimiento:"
                    )
                    
                    if func_idx is None:
                        return None
                        
                    selected_func = motion_library.get_function(func_names[func_idx])
                    
                    # Usar la función para definir velocidades de rotación
                    # Evaluamos la función en varios puntos para estimar velocidades
                    preview = selected_func.preview(10)
                    velocities = np.diff(preview, axis=0).mean(axis=0) * 2 * np.pi
                    
                    speed_x = float(velocities[0])
                    speed_y = float(velocities[1])
                    speed_z = float(velocities[2])
                    
                elif custom_choice == 1:  # Nueva expresión
                    print("\n📝 Escriba expresiones matemáticas para cada eje.")
                    print("Variables disponibles: t (tiempo), theta (ángulo), pi, e")
                    ...
                    try:
                        ...
                        speed_x = float(result[0])
                        speed_y = float(result[1])
                        speed_z = float(result[2])
                    ...
                    print("\n📚 Ejemplos de expresiones para movimiento:")
                    print("\n1. Oscilación simple:")
                    print("   X: sin(t * 2)")
                    print("   Y: cos(t * 2)")
                    print("   Z: 0")
                    ...
                    print("   Y: t * sin(t * 5)")
                    print("   Z: t * 0.5")
                    print("\n3. Figura de Lissajous:")
                    print("   X: sin(3 * t)")
                    print("   Y: sin(2 * t)")
                    print("   Z: sin(4 * t) * 0.5")
                    print("\n4. Péndulo caótico:")
                    print("   X: sin(t) * exp(-t/10)")
                    print("   Y: cos(t * 1.5) * exp(-t/10)")
                    print("   Z: 0")
            ...
            speed = self.ui.get_numeric_input("Velocidad interpolación (0.1-1): ", 0.1, 1) or 0.5
