...
class MacroTrajectory(MotionComponent, MovementModeMixin):
    ...
    def __init__(self):
        ...
        self.speed = 1.0
        self.phase = 0.0
        self.last_center_position = None  # Posición del centro del macro
    ...
    def update(self, state: MotionState, current_time: float, dt: float) -> MotionState:
        ...
        if self.playback_controller:
            ...
            # Calcular incremento de fase basado en velocidad real
            if hasattr(self.trajectory_func, 'length') and self.trajectory_func.length and self.trajectory_func.length > 0:
                # Velocidad constante: m/s → incremento de fase
                distance_to_travel = self.speed * dt  # metros a recorrer
                phase_increment = distance_to_travel / self.trajectory_func.length
            else:
                # Fallback: comportamiento anterior (fase/segundo)
    ...
    def calculate_delta(self, state: 'MotionState', current_time: float, dt: float) -> Optional['MotionDelta']:
        ...
        # La fase ya se actualiza en update(), no duplicar aquí
        ...
        if self.last_center_position is None:
            self.last_center_position = np.zeros(3)
        
        # Calcular delta desde la última posición del CENTRO
        delta_position = target_position - self.last_center_position
        self.last_center_position = target_position.copy()
