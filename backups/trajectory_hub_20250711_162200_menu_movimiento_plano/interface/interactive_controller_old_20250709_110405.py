...
try:
    from trajectory_hub.core.rotation_system import RotationPattern, MacroRotation, TrajectoryRotation
...
try:
    ...
    print("   Verificar que los archivos del núcleo estén presentes")
    sys.exit(1)
from trajectory_hub.core.rotation_system import RotationPattern, MacroRotation, TrajectoryRotation
...
class MockOSCBridge:
    ...
    def __init__(self):
        """Inicializar el controlador interactivo con manejo robusto de errores"""
        print("🚀 Inicializando InteractiveController...")
        ...
        try:
            ...
            try:
                ...
                print("✓ Bridge OSC creado")

            except Exception as e:
                print(f"⚠️  Error creando bridge OSC: {e}")
            ...
            print("✅ InteractiveController inicializado completamente")

        except Exception as e:
            print(f"❌ Error crítico inicializando controlador: {e}")
            logger.error(f"Error crítico: {e}")
            # Crear engine mínimo para evitar crashes
            try:
                self.engine = EnhancedTrajectoryEngine(max_sources=10, fps=30)
                self.bridge = MockOSCBridge()
                self.distance_adjuster = None
                self.running = False
                self.macros = {}
                self.selected_macro = None
                print("⚠️  Controlador inicializado en modo mínimo")
            except Exception as e2:
                print(f"❌ Error crítico fatal: {e2}")
                raise
    def send_position(self, source_id, position):
        """Mock send position"""
        self._stats["messages_sent"] += 1
    
    def send_orientation(self, source_id, yaw, pitch, roll):
        """Mock send orientation"""
        self._stats["messages_sent"] += 1
    
    def send_aperture(self, source_id, aperture):
        """Mock send aperture"""
        self._stats["messages_sent"] += 1
    
    def send_full_state_async(self, positions, orientations, apertures, names):
        """Mock send full state"""
        self._stats["messages_sent"] += len(positions) if positions else 0
        return True
...
class InteractiveController:
    ...
    async def main_menu(self):
        ...
        while self.running:
            ...
            # Macro Source (MS)
            print("\n═══ MACRO SOURCE (MS) ═══════════════════════")
            print("3. MS → Trayectoria Principal")
            print("4. MS → Velocidad Global")
            print("5. MS → Invertir Dirección")
            print("6. MS → Rotar Macro (pitch/yaw/roll)")
            print("7. MS → Rotar Macro (Algorítmico)")
            print("8. MS → Desfase entre Macros (próximamente)")

            # Individual Sources (IS)
            print("\n═══ INDIVIDUAL SOURCES (IS) ═════════════════")
            print("10. IS → Trayectorias Individuales")
            print("11. IS → Velocidades por Fuente")
            print("12. IS → Modos de Movimiento")
            print("13. IS → Desfases Temporales")
            print("14. IS → Pause/Resume Selectivo")
            print("15. IS → Rotar Trayectorias Individuales")
            print("16. IS → Rotar Trayectorias (Algorítmico)")

            # Modulación 3D
            print("\n═══ MODULACIÓN 3D (por fuente) ══════════════")
            print("20. Aplicar Preset de Modulación")
            print("21. Ajustar Velocidad (LFO)")
            print("22. Ajustar Intensidad")
            print("23. Configuración Avanzada")

            # Espacialización
            print("\n═══ ESPACIALIZACIÓN ═════════════════════════")
            print("30. Distancias (MS/IS)")
            print("31. Concentración en Punto (próximamente)")
            print("32. Sistema de Deformación")

            # Composición
            print("\n═══ COMPOSICIÓN ═════════════════════════════")
            print("40. Presets Artísticos")
            print("41. Guardar Estado Actual")
            print("42. Cargar Composición")
            print("43. Interacción entre Macros")
            ...
            try:
                ...
                # MACRO SOURCE (MS)
                    ...
                    await self.ms_rotate_macro()
                elif choice == "7":
                    await self.ms_rotate_algorithmic()
                ...
                # INDIVIDUAL SOURCES (IS)
                    ...
                    await self.is_rotate_trajectories()

                # MODULACIÓN 3D
                elif choice == "16":
                    await self.is_rotate_algorithmic()
                elif choice == "20":
                ...
                # COMPOSICIÓN
                    ...
                    print("\n⚠️  Función en desarrollo: Guardar Estado")
                elif choice == "42":  # Cargar Composición
                    print("\n⚠️  Función en desarrollo: Cargar Composición")
    ...
    async def ms_rotate_algorithmic(self):
        """Aplicar rotación algorítmica a un macro completo"""
        if not self.selected_macro:
            print("\n⚠️  No hay macro seleccionado")
            print("\nPrimero selecciona un macro con la opción 2")
            return
            
        print("\n🔄 ROTACIÓN ALGORÍTMICA MS")
        
        # El ID real del macro en el engine
        macro_id = self.selected_macro  # Este es el ID completo como "macro_0_qwe"
        
        # Buscar el nombre display 
        display_name = None
        for name, stored_id in self.macros.items():
            if stored_id == macro_id:
                display_name = name
                break
                
        print(f"\n📍 Macro seleccionado: {display_name or macro_id}")
        print(f"   ID en engine: {macro_id}")
        ...
        if mode in pattern_map:
            speed = await self._get_input("\nVelocidad de rotación [1.0]: ")
            speed = float(speed) if speed else 1.0
            
            amplitude = await self._get_input("Amplitud [1.0]: ")
            amplitude = float(amplitude) if amplitude else 1.0
            
            from trajectory_hub.core.rotation_system import MacroRotation
            
            macro_rotation = MacroRotation()
            macro_rotation.set_pattern(
                pattern_type=pattern_map[mode],
                speed=speed,
                amplitude=amplitude
            )
            
            # Usar el ID completo del macro (no el nombre display)
            self.engine.apply_algorithmic_rotation_ms(
                macro_name=macro_id,  # Usar el ID real
                macro_rotation=macro_rotation
            )
            
            print(f"\n✅ Rotación '{pattern_map[mode]}' aplicada")
            print(f"   Velocidad: {speed}x, Amplitud: {amplitude}x")
            print("\n💡 Notas:")
            print("   - La rotación es independiente de la trayectoria MS")
            print("   - Usa opción 3 para asignar una trayectoria al macro") 
            print("   - Usa opción 4 para ajustar velocidad de la trayectoria")
        else:
            print("\n❌ Patrón no válido")


    async def is_rotate_algorithmic(self):
        """Aplicar rotación algorítmica a fuentes individuales"""
        ...
        if mode in pattern_map:
            # Importar TrajectoryRotation
            from trajectory_hub.core.rotation_system import TrajectoryRotation
            
            # Pedir rango
            start = await self._get_input("Fuente inicial [0]: ")
            end = await self._get_input("Fuente final [49]: ")
            
            start = int(start) if start else 0
            end = int(end) if end else 49
            
            # Parámetros de rotación
            speed = await self._get_input("Velocidad [1.0]: ")
            speed = float(speed) if speed else 1.0
            
            # Aplicar a cada fuente en el rango
            for source_id in range(start, end + 1):
                traj_rotation = TrajectoryRotation()
                traj_rotation.set_pattern(
                    pattern_type=pattern_map[mode],
                    speed=speed,
                    amplitude=1.0,
                    phase_offset=source_id * 0.1  # Desfase entre fuentes
                )
                
                self.engine.apply_algorithmic_rotation_is(
                    source_id=source_id,
                    traj_rotation=traj_rotation
                )
            
            print(f"\n✅ Rotación '{pattern_map[mode]}' aplicada a fuentes {start}-{end}")
        else:
            print("\n❌ Patrón no válido")
                ...
                if hasattr(self.engine, '_macro_deformers'):
                    if original_id in self.engine._macro_deformers:
                        ...
                        if new_macro_id not in self.engine._macro_deformers:
                            self.engine._macro_deformers[new_macro_id] = ()
    ...
    async def trajectory_control_menu(self):
        ...
        while True:
            ...
            print("8. Rotar macro (pitch/yaw/roll)")
            print("9. Rotar trayectorias individuales")
            print("10. DEBUG - Verificar estado del sistema")
            print("0. Volver")
            ...
            try:
                if choice == "1":
                    ...
                    await self.rotate_macro_menu()
                elif choice == "9":
                    await self.rotate_individual_trajectories_menu()
                elif choice == "10":
    ...
    async def set_macro_trajectory(self):
        """Establecer trayectoria del macro seleccionado"""
        if not self.selected_macro:
        ...
        print("1. Círculo")
        print("2. Espiral")
        print("3. Lissajous (Figura 8)")
        print("4. Infinito")
        print("5. Helicoidal")
        print("6. Rosa")
        print("7. Personalizada (función)")
        
        try:
            choice = await self._get_int("Seleccionar trayectoria: ", 1, 7)
            
            # IMPORTANTE: Inicializar trajectory_func AL PRINCIPIO
            trajectory_func = None
            trajectory_name = ""
            
            # Configuración básica
            print("\n⚙️  CONFIGURACIÓN DE TRAYECTORIA")
            speed = await self._get_float("Velocidad (0.1-5.0, default=1.0): ", 0.1, 5.0) or 1.0
            reverse = await self._get_bool("¿Invertir dirección de giro? (s/n): ")
            scale = await self._get_float("Escala de tamaño (0.5-3.0, default=1.0): ", 0.5, 3.0) or 1.0
            
            # Offset Z
            z_offset = 0.0
            if await self._get_bool("¿Añadir desplazamiento vertical? (s/n): "):
                z_offset = await self._get_float("Desplazamiento Z (-10 a 10): ", -10, 10) or 0.0
            
            # Importar numpy si no está importado
            import numpy as np
            
            # Definir la trayectoria según la elección
            if choice == 1:  # Círculo
                trajectory_name = "circle"
                trajectory_func = lambda t: np.array([
                    scale * 5 * np.cos(t * speed * (-1 if reverse else 1)),
                    scale * 5 * np.sin(t * speed * (-1 if reverse else 1)),
                    z_offset
                ])
                
            elif choice == "16":
                await self.is_rotate_algorithmic()
            elif choice == 2:  # Espiral
                trajectory_name = "spiral"
                trajectory_func = lambda t: np.array([
                    scale * (3 + 0.1 * t) * np.cos(t * speed * (-1 if reverse else 1)),
                    scale * (3 + 0.1 * t) * np.sin(t * speed * (-1 if reverse else 1)),
                    z_offset + 0.2 * t
                ])
                
            elif choice == 3:  # Lissajous
                trajectory_name = "lissajous"
                trajectory_func = lambda t: np.array([
                    scale * 5 * np.cos(2 * np.pi * t * speed),
                    scale * 5 * np.sin(4 * np.pi * t * speed * (-1 if reverse else 1)),
                    z_offset
                ])
                
            elif choice == 4:  # Infinito
                trajectory_name = "infinity"
                trajectory_func = lambda t: np.array([
                    scale * 4 * np.cos(t * speed) / (1 + np.sin(t * speed)**2),
                    scale * 4 * np.sin(t * speed) * np.cos(t * speed) / (1 + np.sin(t * speed)**2) * (-1 if reverse else 1),
                    z_offset
                ])
                
            elif choice == 5:  # Helicoidal
                trajectory_name = "helix"
                trajectory_func = lambda t: np.array([
                    scale * 3 * np.cos(t * speed * (-1 if reverse else 1)),
                    scale * 3 * np.sin(t * speed * (-1 if reverse else 1)),
                    z_offset + 0.5 * t
                ])
                
            elif choice == 6:  # Rosa
                k = await self._get_int("Número de pétalos (2-8): ", 2, 8) or 3
                trajectory_name = "rose"
                trajectory_func = lambda t: np.array([
                    scale * 4 * np.cos(k * t * speed) * np.cos(t * speed),
                    scale * 4 * np.cos(k * t * speed) * np.sin(t * speed) * (-1 if reverse else 1),
                    z_offset
                ])
                
            elif choice == 7:  # Personalizada
                self.ms_rotate_algorithmic()
                return
            
            # VERIFICAR que trajectory_func se definió
            if trajectory_func is None:
                print("\n❌ Error: No se pudo crear la función de trayectoria")
                return
            
            # Preguntar por deformación
            enable_deformation = await self._get_bool("\n¿Activar sistema de deformación? (s/n): ")
            
            # Aplicar la trayectoria
            print(f"\n✅ Aplicando trayectoria '{trajectory_name}' al macro...")
            ...
            print("✅ Trayectoria aplicada exitosamente")
            
            # Si se activó deformación, configurarla
            if enable_deformation:
                print("\n🌊 TIPOS DE DEFORMACIÓN:")
                print("1. Respiración (expansión/contracción)")
                print("2. Ondas")
                print("3. Ninguna por ahora")
                
                deform_choice = await self._get_int("Seleccionar: ", 1, 3)
                
                if deform_choice == 1:
                    period = await self._get_float("Período de respiración (seg): ", 0.5, 10.0) or 4.0
                    amplitude = await self._get_float("Amplitud (0.1-3.0): ", 0.1, 3.0) or 1.0
                    self.engine.apply_breathing(period, amplitude, self.selected_macro)
                    print("✅ Respiración aplicada")
                    
                elif deform_choice == 2:
                    wavelength = await self._get_float("Longitud de onda: ", 1.0, 20.0) or 5.0
                    amplitude = await self._get_float("Amplitud: ", 0.1, 3.0) or 1.0
                    speed = await self._get_float("Velocidad: ", 0.1, 5.0) or 1.0
                    self.engine.apply_wave_deformation(wavelength, amplitude, speed, self.selected_macro)
                    print("✅ Deformación por ondas aplicada")
    ...
    async def configure_individual_trajectories(self):
        """Configurar trayectorias individuales - VERSIÓN COMPLETA Y CORREGIDA"""
        print("\n🎯 TRAYECTORIAS INDIVIDUALES")

        if not self.selected_macro:
            print("\n❌ Error: No hay macro seleccionado")
            return
    ...
    def _prepare_macro_for_individual_trajectories(self, macro):
        """Preparar macro para configuración de trayectorias individuales"""
        ...
        # Asegurar que source_ids existe y es válido
        ...
        # Habilitar trayectorias diferentes si no está habilitado
        if not getattr(macro, 'allow_different_trajectories', False):
            macro.allow_different_trajectories = True
            print("✓ Habilitadas trayectorias individuales para el macro")
    ...
    async def _configure_all_same(self, macro):
        ...
        shape_idx = await self._get_int("\nSeleccionar forma (número): ", 1, len(shapes))
        ...
        print(f"\n✅ {count} fuentes configuradas con trayectoria '{selected_shape}'")
    ...
    async def _configure_mixed_percentages(self, macro):
        ...
        print("\nDefine la distribución de formas (en %)")
        ...
        if distributions:
            ...
            for shape, pct in distributions:
                ...
                print(f"  • {shape}: {count} fuentes ({pct*100:.0f}%)")
    ...
    async def _configure_custom_per_source(self, macro):
        ...
        # Opción de configurar todas o solo algunas
        config_all = await self._get_bool("¿Configurar todas las fuentes? (s/n): ")
        ...
        for i in range(num_to_config):
            ...
            if not config_all:
                idx = await self._get_int(
                    f"Índice de la fuente (1-{len(source_list)}): ",
                    1, len(source_list)
                ) - 1
            else:
                idx = i
        ...
        print(f"\n✅ {configured} fuentes configuradas individualmente")
    ...
    async def _configure_by_groups(self, macro):
        ...
        num_groups = await self._get_int("\n¿Cuántos grupos crear? (2-10): ", 2, min(10, total))
        ...
        print(f"\n✅ {num_groups} grupos configurados")
    ...
    async def _configure_random(self, macro):
        ...
        randomize_shape = await self._get_bool("¿Aleatorizar formas? (s/n): ")
        randomize_mode = await self._get_bool("¿Aleatorizar modos de movimiento? (s/n): ")
        randomize_speed = await self._get_bool("¿Aleatorizar velocidades? (s/n): ")
        ...
        print(f"\n✅ {count} fuentes configuradas aleatoriamente")
    ...
    async def _select_movement_mode(self):
        ...
        movement_modes = [
            ...
            (TrajectoryMovementMode.SPIN, "Giro ultra-rápido"),
            (TrajectoryMovementMode.FREEZE, "Congelado en posición")
        ]
    ...
    async def _get_movement_params(self, mode):
        """Obtener parámetros específicos según el modo de movimiento"""
        params = {}
        ...
        if mode == TrajectoryMovementMode.FIX:
            ...
            params['vibration_frequency'] = await self._get_float(
                "Frecuencia de vibración (0.5-10 Hz): ", 0.5, 10.0
            )
            params['vibration_amplitude'] = await self._get_float(
                "Amplitud de vibración (0.01-1.0): ", 0.01, 1.0
            )
    ...
    def _apply_individual_trajectory(self, source_id, shape, mode, **params):
        """Aplicar configuración de trayectoria individual a una fuente"""
        try:
            # Verificar que el engine tiene el método necesario
            if hasattr(self.engine, 'set_individual_trajectory'):
                ...
                # Intentar aplicar directamente si el método no existe
    ...
    def _apply_distribution(self, macro, distributions):
        """Aplicar distribución de formas a las fuentes"""
        source_list = list(macro.source_ids)
        ...
        for shape, proportion in distributions:
            ...
            for i in range(current_idx, end_idx):
                ...
                self._apply_individual_trajectory(
                    sid, shape, 
                    TrajectoryMovementMode.FIX,
                    movement_movement_movement_speed=1.0
                )
                macro.individual_trajectories[sid] = shape
                ...
                if 'vibration' in params:
                    if hasattr(motion, 'components') and 'individual_trajectory' in motion.components:
                        ...
                        if hasattr(traj, 'set_movement_mode'):
                            from trajectory_hub.core.motion_components import TrajectoryMovementMode, TrajectoryDisplacementMode, TrajectoryDisplacementMode
                            traj.set_movement_mode(
                                TrajectoryMovementMode.VIBRATION,
...
# =====================================
    ...
    async def ms_velocity_menu(self):
        ...
        try:
            ...
            print(f"\n✅ Velocidad establecida a {new_speed:.2f}x")

        except Exception as e:
            print(f"\n❌ Error ajustando velocidad: {e}")


    async def ms_reverse_direction(self):
        """Invertir dirección de todo el macro"""
        if not self.selected_macro:
            print("\n⚠️  No hay macro seleccionado")
            return

        try:
            print("\n🔄 Invirtiendo dirección del macro...")
            ...
            print(f"✅ Dirección {estado}")

        except Exception as e:
            print(f"\n❌ Error invirtiendo dirección: {e}")


    # =====================================
    # MÉTODOS INDIVIDUAL SOURCES (IS)
    # =====================================
    ...
    async def is_pause_resume(self):
        ...
        try:
            ...
            if not source_ids:
                print("\n⚠️  No hay fuentes en este macro")
                return

            print(f"\nFuentes disponibles: {source_ids}")
            print("\n1. Pausar/Reanudar todas")
            print("2. Pausar/Reanudar específicas")
            ...
            if choice == "1":
                ...
                print("\n✅ Estado de todas las fuentes alternado")
                ...
                print(f"\n✅ Estado alternado para fuentes: {selected_ids}")
                ...
                print(f"\n✅ Estado alternado para rango {start}-{end}")
    ...
    # =====================================
    # MS - ROTACIÓN DE MACRO
    # =====================================
    
    async def ms_rotate_macro(self):
        """Rotar un macro completo desde su centro"""
        if not self.selected_macro:
            print("\n⚠️  No hay macro seleccionado")
            return
        ...
        try:
            ...
            pitch = await self._get_float("Pitch (rotación eje X) [-180 a 180°]: ", -180, 180)
            yaw = await self._get_float("Yaw (rotación eje Y) [-180 a 180°]: ", -180, 180)
            roll = await self._get_float("Roll (rotación eje Z) [-180 a 180°]: ", -180, 180)
            ...
            print(f"\n✅ Rotación aplicada: Pitch={pitch:.1f}°, Yaw={yaw:.1f}°, Roll={roll:.1f}°")
            
            if await self._get_bool("\n¿Aplicar otra rotación? (s/n): "):
                await self.ms_rotate_macro()
                
        except Exception as e:
            print(f"\n❌ Error aplicando rotación: {e}")
    
    # =====================================
    # IS - ROTACIÓN DE TRAYECTORIAS
    # =====================================

    async def is_rotate_trajectories(self):
        """Rotar trayectorias individuales - Versión corregida"""
        if not self.selected_macro:
            print("\n⚠️  No hay macro seleccionado")
            return
        
        print("\n🔄 ROTAR TRAYECTORIAS INDIVIDUALES (IS)")
        print("-" * 40)
        
        try:
            # Obtener el macro y sus fuentes
            macro = self.engine._macros[self.selected_macro]
            source_ids = list(self._get_macro_source_ids(macro))
            ...
            print(f"\nFuentes disponibles: {len(source_ids)}")
            print("\n1. Rotar todas con los mismos ángulos")
            print("2. Rotar fuentes específicas")
            print("3. Rotación aleatoria")
            print("0. Volver")
            ...
            if choice == "1":
                # Rotación uniforme
                print("\n📐 ROTACIÓN UNIFORME")
                pitch = await self._get_float("Pitch [-180 a 180°]: ", -180, 180)
                yaw = await self._get_float("Yaw [-180 a 180°]: ", -180, 180)
                roll = await self._get_float("Roll [-180 a 180°]: ", -180, 180)
                
                # Aplicar rotación directamente al engine
                import numpy as np
                success_count = 0
                
                for source_id in source_ids:
                    try:
                        self.engine.set_trajectory_rotation(
                            source_id,
                            np.radians(pitch),
                            np.radians(yaw),
                            np.radians(roll)
                        ...
                        print(f"⚠️  Error en fuente {source_id}: {e}")
                
                print(f"\n✅ Rotación aplicada a {success_count}/{len(source_ids)} trayectorias")
                
            elif choice == "2":
                # Rotación de fuentes específicas
                print(f"\nFuentes: {source_ids}")
                ids_str = await self._get_input("IDs a rotar (separados por comas): ")
                ...
                try:
                    ...
                    # Filtrar solo los que pertenecen al macro
                    valid_ids = [sid for sid in selected_ids if sid in source_ids]
                    
                    if not valid_ids:
                        print("\n⚠️  Ningún ID válido seleccionado")
                        return
                    ...
                    print(f"\n✅ Rotación aplicada a {success_count}/{len(valid_ids)} fuentes")
                ...
                # Rotación aleatoria
                max_angle = await self._get_float("Ángulo máximo aleatorio [0-180°]: ", 0, 180)
                ...
                for source_id in source_ids:
                    try:
                        ...
                        yaw = random.uniform(-max_angle, max_angle)
                        roll = random.uniform(-max_angle/2, max_angle/2)  # Roll más limitado
                ...
                print(f"\n✅ Rotación aleatoria aplicada a {success_count}/{len(source_ids)} trayectorias")
...
if __name__ == "__main__":
    ...
    async def save_current_state(self):
        """Guardar el estado actual como composición"""
        print("\n💾 GUARDAR ESTADO ACTUAL")
        print("-" * 40)

        try:
            name = await self._get_input("Nombre de la composición: ")
            ...
            state = {
                ...
                'description': await self._get_input("Descripción (opcional): ")
            }
            ...
            print(f"\n✅ Composición '{name}' guardada exitosamente")

        except Exception as e:
            print(f"\n❌ Error guardando composición: {e}")

    async def load_composition(self):
        """Cargar una composición guardada"""
        print("\n📂 CARGAR COMPOSICIÓN")
        print("-" * 40)
        ...
        try:
            ...
            if not saves_dir.exists():
                print("\n⚠️  No hay composiciones guardadas")
                return
            ...
            idx = await self._get_int("Seleccionar composición: ", 1, len(files))
            ...
            print(f"\nCargando composición '{selected_file.stem}'...")
            ...
            # TODO: Implementar la lógica de carga real
            print("\n⚠️  Función de carga en desarrollo")
    ...
    async def rotate_macro_menu(self):
        """Menú para rotar un macro completo"""
        print("\n🔄 ROTAR MACRO")
        print("-" * 40)
        ...
        print("\nRotar el macro completo desde su centro")
        print("Los valores son en grados (se convertirán a radianes)")
        print("Valores positivos = sentido horario")
        ...
        try:
            ...
            # Aplicar rotación
            self.engine.set_macro_rotation(self.selected_macro, pitch_rad, yaw_rad, roll_rad)

            print(f"\n✅ Rotación aplicada: Pitch={pitch:.1f}°, Yaw={yaw:.1f}°, Roll={roll:.1f}°")

            # Preguntar si quiere acumular más rotaciones
            if await self._get_bool("\n¿Aplicar otra rotación? (s/n): "):
                await self.rotate_macro_menu()
    ...
    async def rotate_individual_trajectories_menu(self):
        """Menú para rotar trayectorias individuales"""
        print("\n🔄 ROTAR TRAYECTORIAS INDIVIDUALES")
        print("-" * 40)
        ...
        print(f"\nFuentes disponibles: {len(source_ids)}")
        print("\n1. Rotar todas las trayectorias igual")
        print("2. Rotar fuentes específicas")
        print("3. Rotación aleatoria para cada fuente")
        print("0. Volver")
        ...
        try:
            if choice == "1":
                ...
                # Convertir a radianes y aplicar
                for sid in source_ids:
                    self.engine.set_individual_rotation(
                        sid, 
                        np.radians(pitch), 
                        np.radians(yaw), 
                        np.radians(roll)
                ...
                print(f"\n✅ Rotación aplicada a {len(source_ids)} fuentes")

            elif choice == "2":
                # Rotar específicas
                print(f"\nFuentes: {source_ids}")
                selection = await self._get_input("IDs a rotar (separados por comas): ")
                ...
                if selected_ids:
                    ...
                    print(f"\n✅ Rotación aplicada a {len(selected_ids)} fuentes")
                else:
                    print("\n⚠️  No se seleccionaron fuentes válidas")
                ...
                print(f"\n✅ Rotación aleatoria aplicada a {count} fuentes")
                print(f"   Rango: ±{max_angle:.1f}° en cada eje")
