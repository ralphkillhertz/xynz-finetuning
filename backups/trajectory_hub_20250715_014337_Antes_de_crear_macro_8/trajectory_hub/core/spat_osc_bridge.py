...
class SpatOSCBridge:
    ...
    def send_source_orientation(self, source_id: int, yaw: float, pitch: float, roll: float):
        """
        Envía orientación (yaw, pitch, roll) de una fuente a Spat
        
        Args:
            source_id: ID de la fuente (1-based para Spat)
            yaw: Ángulo yaw en grados
            pitch: Ángulo pitch en grados  
            roll: Ángulo roll en grados
        """
        if 1 <= source_id <= self.n_sources:
            address = f"/source/{source_id}/ypr"
            # Convertir a float para evitar problemas de tipo
            yaw_val = float(yaw) if not np.isnan(yaw) else 0.0
            pitch_val = float(pitch) if not np.isnan(pitch) else 0.0
            roll_val = float(roll) if not np.isnan(roll) else 0.0
            
            self.client.send_message(address, [yaw_val, pitch_val, roll_val])
    
    def send_source_orientations(self, orientations: Dict[int, Tuple[float, float, float]]):
        """
        Envía múltiples orientaciones de fuentes
        
        Args:
            orientations: Dict con source_idx (0-based) -> (yaw, pitch, roll) en grados
        """
        for source_idx, (yaw, pitch, roll) in orientations.items():
            # Convertir a 1-based para Spat
            source_id = source_idx + 1
            self.send_source_orientation(source_id, yaw, pitch, roll)
