...
class PlaybackController:
    ...
    def __init__(self):
        ...
        self.trajectory_func = None  # Referencia a la función de trayectoria para obtener longitud
    ...
    def _update_fix_mode(self, delta_time: float, current_t: float) -> float:
        ...
        # Usar longitud de trayectoria si está disponible para velocidad real en m/s
        if (self.trajectory_func and 
            hasattr(self.trajectory_func, 'length') and 
            self.trajectory_func.length and 
            self.trajectory_func.length > 0):
            # Velocidad real en m/s
            distance_to_travel = self.base_speed * delta_time
            phase_increment = distance_to_travel / self.trajectory_func.length
        else:
            # Fallback: comportamiento anterior (velocidad normalizada)
            phase_increment = self.base_speed * delta_time
        
        new_t = current_t + phase_increment
        return new_t % 1.0
