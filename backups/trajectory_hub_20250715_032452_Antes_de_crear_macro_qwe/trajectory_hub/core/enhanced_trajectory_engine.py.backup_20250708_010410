...
from trajectory_hub.core.motion_components import (
    ...
    TrajectoryTransform,
    MacroTrajectory,
# REMOVED:     create_complex_movement,
    MotionState,
...
class EnhancedTrajectoryEngine:
    ...
    def set_individual_trajectory(self, source_id: int, shape: str, 
        ...
        from trajectory_hub.core.motion_components import MotionDelta, IndividualTrajectory
        ...
        for macro_id, macro in self._macros.items():
            if hasattr(macro, 'trajectory_component') and macro.trajectory_component and macro.trajectory_component.enabled:
                ...
                if macro.source_ids:
                    ...
                    if positions:
                        import numpy as np
                        from trajectory_hub.core.motion_components import MotionDelta, MotionState
    ...
    def _send_osc_update(self):
        ...
        for source_id, motion in self.motion_states.items():
            if hasattr(motion, 'update_with_deltas'):
                deltas = motion.update_with_deltas(time.time(), 1.0/self._update_rate)
                all_deltas.extend(deltas)
        
        # Aplicar todos los deltas
        for delta in all_deltas:
            if delta.source_id < len(self._positions):
                if delta.position is not None:
                    self._positions[delta.source_id] += delta.position
        ...
        for source_id in macro.source_ids:
            if source_id in self._source_motions:
                ...
                if 'individual_trajectory' in motion.components:
                    traj = motion.components['individual_trajectory']
                    try:
                        from trajectory_hub.core.motion_components import MotionDelta, TrajectoryMovementMode
