...
def _create_circle_trajectory(**kwargs) -> Callable[[float], np.ndarray]:
    ...
    # Longitud real de la circunferencia en metros
    trajectory.length = 2 * np.pi * radius
    return trajectory
...
def _create_spiral_trajectory(**kwargs) -> Callable[[float], np.ndarray]:
    ...
    # Aproximación de longitud de espiral en metros
    trajectory.length = np.sqrt((2 * np.pi * radius * turns)**2 + height**2)
    return trajectory
...
def _create_figure8_trajectory(**kwargs) -> Callable[[float], np.ndarray]:
    ...
    # Aproximación de longitud para figura 8 en metros
    trajectory.length = 5.24 * scale
    return trajectory
...
def _create_lissajous_trajectory(**kwargs) -> Callable[[float], np.ndarray]:
    ...
    # Aproximación de longitud para Lissajous en metros
    trajectory.length = 4 * scale * max(freq_x, freq_y, 1.0)
    return trajectory
...
def _create_random_trajectory(**kwargs) -> Callable[[float], np.ndarray]:
    ...
    # Aproximación de longitud para trayectoria aleatoria en metros
    trajectory.length = scale * num_points * 1.5
    return trajectory
...
def _create_helix_trajectory(**kwargs) -> Callable[[float], np.ndarray]:
    ...
    # Longitud de hélice en metros
    trajectory.length = turns * np.sqrt((2 * np.pi * radius)**2 + pitch**2)
    return trajectory
...
def _create_line_trajectory(**kwargs) -> Callable[[float], np.ndarray]:
    ...
    # Longitud de línea recta en metros
    trajectory.length = np.linalg.norm(end - start)
    return trajectory
...
def _create_wave_trajectory(**kwargs) -> Callable[[float], np.ndarray]:
    ...
    # Longitud aproximada para onda sinusoidal en metros
    trajectory.length = length * (1 + (amplitude * frequency * 0.5) / length)
    return trajectory
...
def _create_torus_knot_trajectory(**kwargs) -> Callable[[float], np.ndarray]:
    ...
    # Aproximación de longitud para nudo toroidal en metros
    trajectory.length = 2 * np.pi * np.sqrt(R**2 + r**2) * max(p, q)
    return trajectory
...
def _create_legacy_wrapper(trajectory_type: str, **kwargs) -> Callable[[float], np.ndarray]:
    ...
    # Longitudes aproximadas para funciones legacy en metros
    if trajectory_type == "spiral":
        trajectory.length = scale * 8.0  # Espiral expansiva
    else:
        trajectory.length = scale * 4.0  # Figuras cerradas
    return trajectory
