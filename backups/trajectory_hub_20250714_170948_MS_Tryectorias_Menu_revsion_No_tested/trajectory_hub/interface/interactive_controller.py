...
class InteractiveController:
    ...
    def _show_movement_menu(self):
        ...
        # Mostrar información del macro activo
        if self.selected_macro and self.selected_macro in self.engine._macros:
            macro = self.engine._macros[self.selected_macro]
            print(f"\nMacro activo: {macro.name} ({len(macro.source_ids)} fuentes)")
        
        options = [
            ("MACRO (MS):", ""),
            ("1", "🔄 Trayectorias Macro"),
            ("2", "🌀 Rotaciones Macro"),
            ("", ""),
            ("INDIVIDUAL (IS):", ""),
            ("3", "🎯 Trayectorias Individuales"),
            ("4", "🔸 Rotaciones Individuales"),
            ("", ""),
            ("EFECTOS GLOBALES:", ""),
            ("5", "📍 Concentración/Dispersión"),
            ("6", "🎭 Modulación 3D"),
            ("-", ""),
            ("b", "Volver"),
            ("q", "Salir")
    ...
    def _process_main_choice(self, choice: str):
        ...
        if choice == "1":
            ...
            self.current_menu = "movement"  # Cambiado de "delta" a "movement"
        elif choice == "3":
    ...
    def _process_movement_choice(self, choice: str):
        ...
        if choice == "1":
            self._show_macro_trajectory_menu()
        elif choice == "2":
            self._show_macro_rotation_menu()
        elif choice == "3":
            if not self._ensure_macro_selected():
                return
            command = self._create_individual_trajectory_command()
            if command:
                self._execute_command(command)
        elif choice == "4":
            if not self._ensure_macro_selected():
                return
            command = self._create_individual_rotation_command()
            if command:
                self._execute_command(command)
        elif choice == "5":
            if not self._ensure_macro_selected():
                return
            command = self._create_concentration_command()
            if command:
                self._execute_command(command)
        elif choice == "6":
            if not self._ensure_macro_selected():
                return
            self._show_modulation_menu()
    ...
    def _create_macro_trajectory_command(self) -> Optional[SemanticCommand]:
        ...
        trajectory_params = config.get("trajectory_params", {})
        trajectory_params["speed"] = config["speed"]  # Asegurar que speed esté incluido
        
        # Añadir parámetros de playback si existen
        if "playback_mode" in config:
            trajectory_params["playback_mode"] = config["playback_mode"]
        if "playback_params" in config:
            trajectory_params["playback_params"] = config["playback_params"]
    ...
    def _show_active_modulations(self):
        """Muestra las modulaciones activas"""
        # TODO: Implementar visualización de modulaciones activas
        self.ui.show_info("Visualización de modulaciones activas en desarrollo")
    
    def _toggle_freeze_trajectory(self):
        """Congela o descongela la trayectoria del macro seleccionado"""
        if not self.selected_macro:
            self.ui.show_error("No hay macro seleccionado")
            return
            
        # Verificar si el macro tiene trayectoria
        macro = self.engine._macros.get(self.selected_macro)
        if not macro or not macro.trajectory_component or not macro.trajectory_component.enabled:
            self.ui.show_error("El macro no tiene trayectoria activa")
            return
            
        # Verificar si hay un controlador de playback
        controller = self.engine.playback_manager.get_controller(self.selected_macro)
        if not controller:
            self.ui.show_error("No hay controlador de reproducción para este macro")
            return
            
        # Toggle freeze state
        if controller.is_frozen:
            self.engine.unfreeze_macro(self.selected_macro)
            self.ui.show_success(f"✅ Macro '{self.selected_macro}' descongelado")
        else:
            self.engine.freeze_macro(self.selected_macro)
            self.ui.show_success(f"⏸️ Macro '{self.selected_macro}' congelado")
    
    def _change_playback_mode(self):
        """Cambia el modo de reproducción de la trayectoria"""
        if not self.selected_macro:
            self.ui.show_error("No hay macro seleccionado")
            return
        ...
        mode_idx = self.ui.get_numeric_input("\nSeleccione modo (1-4): ", 1, 4)
        if mode_idx is None:
            return
            
        selected_mode = playback_modes[int(mode_idx)-1][0]
        
        # Obtener parámetros específicos del modo
        params = {}
        
        if selected_mode == "random":
            print("\n  Configuración modo Random:")
            params["change_interval"] = self.ui.get_numeric_input(
                "    Intervalo de cambio (segundos): ", 0.5, 10.0
            ) or 2.0
            min_speed = self.ui.get_numeric_input("    Velocidad mínima: ", 0.1, 5.0) or 0.1
            max_speed = self.ui.get_numeric_input("    Velocidad máxima: ", 0.1, 5.0) or 3.0
            params["speed_range"] = (min_speed, max_speed)
            
        elif selected_mode == "vibration":
            print("\n  Configuración modo Vibration:")
            params["amplitude"] = self.ui.get_numeric_input(
                "    Amplitud de vibración: ", 0.01, 0.2
            ) or 0.05
            params["frequency"] = self.ui.get_numeric_input(
                "    Frecuencia (Hz): ", 1.0, 50.0
            ) or 10.0
            
        elif selected_mode == "spin":
            print("\n  Configuración modo Spin:")
            params["base_speed"] = self.ui.get_numeric_input(
                "    Velocidad base: ", 0.1, 5.0
            ) or 1.0
            params["variation"] = self.ui.get_numeric_input(
        ...
        # Aplicar el modo
        self.engine.set_macro_playback_mode(self.selected_macro, selected_mode, **params)
        self.ui.show_success(f"✅ Modo '{selected_mode}' aplicado al macro '{self.selected_macro}'")
    
    def _show_macro_trajectory_menu(self):
        """Menú de trayectorias macro"""
        if not self._ensure_macro_selected():
            return
            
        # Obtener información actual de la trayectoria
        macro = self.engine._macros[self.selected_macro]
        trajectory_info = self._get_trajectory_info(macro)
        
        # Mostrar estado actual
        print(f"\nMacro: {macro.name} | Tipo: {trajectory_info['type']} | Vel: {trajectory_info['speed']} | {trajectory_info['size_info']}")
        print(f"Estado: {trajectory_info['state']} | Modo: {trajectory_info['mode']}")
        
        options = [
            ("CREAR NUEVA:", ""),
            ("1", "⚡ Trayectoria rápida (presets)"),
            ("2", "🔧 Trayectoria detallada"),
            ("", ""),
            ("EDITAR ACTUAL:", ""),
            ("3", f"⚡ Velocidad               [{trajectory_info['speed']}]"),
            ("4", f"📏 Tamaño/Radio            [{trajectory_info['size']}]"),
            ("5", f"🎮 Modo reproducción       [{trajectory_info['mode']}]"),
            ("6", "🔧 Parámetros específicos"),
            ("7", f"{trajectory_info['freeze_icon']} {trajectory_info['freeze_text']}"),
            ("", ""),
            ("CAMBIAR:", ""),
            ("8", "🔄 Cambiar tipo de trayectoria"),
            ("-", ""),
            ("b", "Volver"),
            ("q", "Salir")
        ]
        
        self.ui.show_menu("TRAYECTORIAS MACRO (MS)", options)
        choice = self.ui.get_input()
        self._process_macro_trajectory_choice(choice)
    
    def _show_macro_rotation_menu(self):
        """Menú de rotaciones macro"""
        if not self._ensure_macro_selected():
            return
            
        # Obtener información actual de la rotación
        macro = self.engine._macros[self.selected_macro]
        rotation_info = self._get_rotation_info(macro)
        
        # Mostrar estado actual
        print(f"\nMacro: {macro.name} | Rotación: {rotation_info['type']}")
        print(f"Velocidades: X:{rotation_info['speed_x']} Y:{rotation_info['speed_y']} Z:{rotation_info['speed_z']} rad/s")
        
        options = [
            ("CREAR NUEVA:", ""),
            ("1", "🔄 Rotación algorítmica"),
            ("2", "🎯 Rotación manual"),
            ("", ""),
            ("EDITAR ACTUAL:", ""),
            ("3", f"↔️  Velocidad X          [{rotation_info['speed_x']}]"),
            ("4", f"↕️  Velocidad Y          [{rotation_info['speed_y']}]"),
            ("5", f"🔄 Velocidad Z          [{rotation_info['speed_z']}]"),
            ("6", "📍 Centro de rotación"),
            ("", ""),
            ("CONTROL:", ""),
            ("7", f"{rotation_info['toggle_icon']} {rotation_info['toggle_text']}"),
            ("8", "🔄 Cambiar tipo"),
            ("-", ""),
            ("b", "Volver"),
            ("q", "Salir")
        ...
        self.ui.show_menu("ROTACIONES MACRO (MS)", options)
        choice = self.ui.get_input()
        self._process_macro_rotation_choice(choice)
    
    def _show_modulation_menu(self):
        """Menú de modulación 3D"""
        options = [
            ("1", "🌊 Aplicar preset de modulación"),
            ("2", "🎛️ Configurar parámetros"),
            ("3", "📊 Ver modulaciones activas"),
            ("4", "🔧 Crear modulación personalizada"),
            ("-", ""),
            ("b", "Volver"),
            ("q", "Salir")
        ]
        
        self.ui.show_menu("MODULACIÓN 3D", options)
        choice = self.ui.get_input()
        
        if choice == "1":
            command = self._create_modulation_command()
            if command:
                self._execute_command(command)
        elif choice == "2":
            self.ui.show_info("Configuración de parámetros no implementada aún")
        elif choice == "3":
            self._show_active_modulations()
        elif choice == "4":
            self.ui.show_info("Creación personalizada no implementada aún")
    
    # ========== UTILIDADES ==========
    
    def _get_trajectory_info(self, macro) -> dict:
        """Obtiene información de la trayectoria actual del macro"""
        info = {
            'type': 'ninguna',
            'speed': '0',
            'size': 'N/A',
            'size_info': '',
            'mode': 'N/A',
            'state': '⏹️ Sin trayectoria',
            'freeze_icon': '⏸️',
            'freeze_text': 'Congelar trayectoria'
        }
        
        if hasattr(macro, 'trajectory_component') and macro.trajectory_component and macro.trajectory_component.enabled:
            # Tipo de trayectoria
            if hasattr(macro.trajectory_component.trajectory_func, 'trajectory_type'):
                info['type'] = macro.trajectory_component.trajectory_func.trajectory_type
            
            # Velocidad
            info['speed'] = f"{macro.trajectory_component.speed:.1f}"
            
            # Tamaño/escala (esto dependerá del tipo)
            # Por ahora usar un valor genérico
            info['size'] = "5m"
            info['size_info'] = "Radio: 5m"
            
            # Modo de reproducción
            controller = self.engine.playback_manager.get_controller(self.selected_macro)
            if controller:
                info['mode'] = controller.mode.value.capitalize()
                if controller.is_frozen:
                    info['state'] = '⏸️ Congelado'
                    info['freeze_icon'] = '▶️'
                    info['freeze_text'] = 'Descongelar trayectoria'
                else:
                    info['state'] = '▶️ En movimiento'
            else:
                info['mode'] = 'Fix'
                info['state'] = '▶️ En movimiento'
    ...
    def _get_rotation_info(self, macro) -> dict:
        """Obtiene información de la rotación actual del macro"""
        info = {
            'type': 'Ninguna',
            'speed_x': '0.0',
            'speed_y': '0.0', 
            'speed_z': '0.0',
            'toggle_icon': '⏸️',
            'toggle_text': 'Activar rotación'
        }
        
        # Buscar si hay rotación activa
        for sid in macro.source_ids:
            if sid in self.engine.motion_states:
                motion = self.engine.motion_states[sid]
                if 'macro_rotation' in motion.active_components:
                    rotation = motion.components['macro_rotation']
                    info['type'] = 'Algorítmica'
                    info['speed_x'] = f"{rotation.speed_x:.1f}"
                    info['speed_y'] = f"{rotation.speed_y:.1f}"
                    info['speed_z'] = f"{rotation.speed_z:.1f}"
                    info['toggle_icon'] = '⏸️'
                    info['toggle_text'] = 'Desactivar rotación'
                    break
        
        return info
    
    def _process_macro_trajectory_choice(self, choice: str):
        """Procesa las opciones del menú de trayectorias macro"""
        if choice == "1":
            self._create_trajectory_quick()
        elif choice == "2":
            command = self._create_macro_trajectory_command()
            if command:
                self._execute_command(command)
        elif choice == "3":
            self._edit_trajectory_speed()
        elif choice == "4":
            self._edit_trajectory_scale()
        elif choice == "5":
            self._change_playback_mode()
        elif choice == "6":
            self._edit_trajectory_params()
        elif choice == "7":
            self._toggle_freeze_trajectory()
        elif choice == "8":
            self._change_trajectory_type()
        elif choice == "b":
            return
    ...
    def _process_macro_rotation_choice(self, choice: str):
        """Procesa las opciones del menú de rotaciones macro"""
        if choice == "1":
            command = self._create_macro_rotation_command(algorithmic=True)
            if command:
                self._execute_command(command)
        elif choice == "2":
            command = self._create_macro_rotation_command(manual=True)
            if command:
                self._execute_command(command)
        elif choice == "3":
            self._edit_rotation_speed('x')
        elif choice == "4":
            self._edit_rotation_speed('y')
        elif choice == "5":
            self._edit_rotation_speed('z')
        elif choice == "6":
            self._edit_rotation_center()
        elif choice == "7":
            self._toggle_rotation()
        elif choice == "8":
            self._change_rotation_type()
        elif choice == "b":
            return
    ...
    def _create_trajectory_quick(self):
        """Crear trayectoria con valores predefinidos"""
        print("\n" + "="*60)
        print("CREAR TRAYECTORIA RÁPIDA".center(60))
        print("="*60)
        
        options = [
            ("CIRCULARES (cerradas):", ""),
            ("1", "⭕ Círculo      - Radio: 5m"),
            ("2", "∞  Figura 8     - Tamaño: 5m"),
            ("3", "🍥 Espiral      - Radio: 3m, Altura: 10m"),
            ("", ""),
            ("PARAMÉTRICAS:", ""),
            ("4", "〰️  Lissajous    - Tamaño: 5m, Frecuencias: 3,2,4"),
            ("5", "🎲 Aleatorio    - Rango: 5m"),
            ("", ""),
            ("LINEALES (abiertas):", ""),
            ("6", "➖ Línea        - Longitud: 10m"),
            ("7", "〰️  Onda         - Longitud: 20m, Amplitud: 3m"),
            ("8", "🌀 Hélice       - Radio: 3m, Paso: 2m"),
            ("", ""),
            ("ESPECIALES:", ""),
            ("9", "🔗 Nudo Toro    - Radio mayor: 5m, menor: 2m"),
            ("-", ""),
            ("b", "Volver")
        ]
        ...
        print("\nVelocidad inicial: 1.0 (ajustable después)")
        
        choice = self.ui.get_input("\nSelección: ")
        
        # Mapeo de opciones a configuraciones predefinidas
        presets = {
            "1": {"type": "circle", "params": {"radius": 5.0, "plane": "xy"}},
            "2": {"type": "figure8", "params": {"scale": 5.0}},
            "3": {"type": "spiral", "params": {"radius": 3.0, "height": 10.0, "turns": 3.0}},
            "4": {"type": "lissajous", "params": {"scale": 5.0, "freq_x": 3.0, "freq_y": 2.0, "freq_z": 4.0}},
            "5": {"type": "random", "params": {"scale": 5.0, "seed": 42, "num_points": 10}},
            "6": {"type": "line", "params": {"start": np.array([0.0, 0.0, 0.0]), "end": np.array([10.0, 0.0, 0.0])}},
            "7": {"type": "wave", "params": {"length": 20.0, "amplitude": 3.0, "frequency": 2.0, "axis": "x", "wave_axis": "y"}},
            "8": {"type": "helix", "params": {"radius": 3.0, "pitch": 2.0, "turns": 3.0}},
            "9": {"type": "torus_knot", "params": {"major_radius": 5.0, "minor_radius": 2.0, "p": 2, "q": 3}}
        }
        ...
        if choice in presets:
            ...
            params["speed"] = 1.0
            params["playback_mode"] = "fix"  # Asegurar modo Fix por defecto
            ...
            self._execute_command(command)
            self.ui.show_success(f"✅ Trayectoria '{preset['type']}' creada con valores predefinidos")
    
    def _edit_trajectory_speed(self):
        """Editar velocidad de la trayectoria actual"""
        macro = self.engine._macros[self.selected_macro]
        
        if not hasattr(macro, 'trajectory_component') or not macro.trajectory_component or not macro.trajectory_component.enabled:
            self.ui.show_error("No hay trayectoria activa para editar")
            return
            
        current_speed = macro.trajectory_component.speed
        
        print("\n" + "="*60)
        print("EDITAR VELOCIDAD".center(60))
        print("="*60)
        print(f"Macro: {macro.name}")
        print(f"Trayectoria: {getattr(macro.trajectory_component.trajectory_func, 'trajectory_type', 'desconocida')}")
        print(f"\nVelocidad actual: {current_speed}")
        print("  • Positiva: movimiento normal")
        print("  • Negativa: movimiento inverso")
        print("  • 0: detenido")
        
        new_speed = self.ui.get_numeric_input("\nNueva velocidad (-10 a 10) [Enter = mantener]: ", -10.0, 10.0)
        
        if new_speed is not None:
            self.engine.update_trajectory_speed(self.selected_macro, new_speed)
            self.ui.show_success(f"✅ Velocidad actualizada a {new_speed}")
    
    def _edit_trajectory_scale(self):
        """Editar tamaño/escala de la trayectoria actual"""
        macro = self.engine._macros[self.selected_macro]
        ...
        traj_type = getattr(macro.trajectory_component.trajectory_func, 'trajectory_type', 'desconocida')
        
        print("\n" + "="*60)
        print("EDITAR TAMAÑO/ESCALA".center(60))
        print("="*60)
        print(f"Macro: {macro.name}")
        print(f"Trayectoria: {traj_type}")
        
        # Determinar qué parámetro editar según el tipo
        param_name = "escala"
        current_value = 5.0  # Valor por defecto
        
        if traj_type in ["circle", "spiral", "helix"]:
            param_name = "radio"
        elif traj_type in ["line", "wave"]:
            param_name = "longitud"
        elif traj_type == "torus_knot":
            param_name = "radio mayor"
            
        print(f"\n{param_name.capitalize()} actual: {current_value}m")
        
        new_value = self.ui.get_numeric_input(f"\nNuevo {param_name} (0.1 a 50) [Enter = mantener]: ", 0.1, 50.0)
        
        if new_value is not None:
            self.engine.update_trajectory_scale(self.selected_macro, new_value)
            self.ui.show_success(f"✅ {param_name.capitalize()} actualizado a {new_value}m")
    
    def _edit_trajectory_params(self):
        """Editar parámetros específicos de la trayectoria"""
        macro = self.engine._macros[self.selected_macro]
        ...
        print("\n" + "="*60)
        print(f"PARÁMETROS ESPECÍFICOS - {traj_type.upper()}".center(60))
        print("="*60)
        print(f"Macro: {macro.name}")
        
        # TODO: Implementar edición de parámetros específicos según tipo
        self.ui.show_info("Edición de parámetros específicos en desarrollo")
    
    def _change_trajectory_type(self):
        """Cambiar el tipo de trayectoria manteniendo velocidad si es posible"""
        current_speed = 1.0
        macro = self.engine._macros[self.selected_macro]
        ...
        print("\n" + "="*60)
        print("CAMBIAR TIPO DE TRAYECTORIA".center(60))
        print("="*60)
        print(f"Velocidad actual se mantendrá: {current_speed}")
        
        # Reutilizar el menú de creación rápida
        self._create_trajectory_quick()
        
        # Actualizar la velocidad después de crear la nueva trayectoria
        if hasattr(macro, 'trajectory_component') and macro.trajectory_component:
            macro.trajectory_component.speed = current_speed
    
    def _edit_rotation_speed(self, axis: str):
        """Editar velocidad de rotación en un eje específico"""
        macro = self.engine._macros[self.selected_macro]
        rotation_info = self._get_rotation_info(macro)
        
        if rotation_info['type'] == 'Ninguna':
            self.ui.show_error("No hay rotación activa para editar")
            return
            
        current_speed = float(rotation_info[f'speed_{axis}'])
        
        print("\n" + "="*60)
        print(f"EDITAR VELOCIDAD {axis.upper()}".center(60))
        print("="*60)
        print(f"Macro: {macro.name}")
        print(f"Velocidad {axis.upper()} actual: {current_speed} rad/s")
        
        new_speed = self.ui.get_numeric_input(f"\nNueva velocidad {axis.upper()} (-5 a 5) [Enter = mantener]: ", -5.0, 5.0)
        
        if new_speed is not None:
            speeds = {
                'speed_x': float(rotation_info['speed_x']),
                'speed_y': float(rotation_info['speed_y']),
                'speed_z': float(rotation_info['speed_z'])
            }
            speeds[f'speed_{axis}'] = new_speed
            
            self.engine.set_macro_rotation(
                self.selected_macro,
                speed_x=speeds['speed_x'],
                speed_y=speeds['speed_y'],
                speed_z=speeds['speed_z']
            )
            self.ui.show_success(f"✅ Velocidad {axis.upper()} actualizada a {new_speed} rad/s")
    
    def _edit_rotation_center(self):
        """Editar el centro de rotación"""
        # TODO: Implementar
        self.ui.show_info("Edición de centro de rotación en desarrollo")
    ...
    def _toggle_rotation(self):
        ...
        if rotation_info['type'] == 'Ninguna':
            # Crear nueva rotación con valores por defecto
            self.engine.set_macro_rotation(self.selected_macro, 0.5, 0.5, 0.0)
            self.ui.show_success("✅ Rotación activada")
        else:
            # Desactivar poniendo velocidades a 0
            self.engine.set_macro_rotation(self.selected_macro, 0.0, 0.0, 0.0)
            self.ui.show_success("⏸️ Rotación desactivada")
    
    def _change_rotation_type(self):
        """Cambiar entre rotación algorítmica y manual"""
        # TODO: Implementar
        self.ui.show_info("Cambio de tipo de rotación en desarrollo")
