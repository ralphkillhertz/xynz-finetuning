    ...
    def add_component(self, component, name=None):
        ...
        if component not in self.active_components:
            ...
            if component_type.endswith('component'):
                component_type = component_type[:-9]  # Quitar 'component'
            self.active_components[component_type] = component
    ...
    def update(self, current_time: float, dt: float) -> MotionState:
        ...
        for component in self.active_components.values():
            if hasattr(component, 'update'):
                ...
                # Llamar update con la firma correcta
                if component.__class__.__name__ in ['ManualMacroRotation', 'ManualIndividualRotation']:
                    # Estos componentes esperan (current_time, dt, state)
                    new_state = component.update(current_time, dt, self.state)
                elif component.__class__.__name__ == 'IndividualRotation':
                    # Este componente espera solo (current_time, dt)
                    component.update(current_time, dt)
                    new_state = self.state
                else:
                    # La mayoría espera (state, current_time, dt)
        ...
        if isinstance(self.active_components, dict):
            for name, component in self.active_components.items():
                if component and hasattr(component, 'enabled') and component.enabled:
                    # DEBUG: Log para ver qué componentes están activos
                    if name == "macro_trajectory":
                        print(f"[DEBUG] Procesando {name} para source {self.source_id}, enabled: {component.enabled}")
                    
                    # Primero llamar a update() para actualizar estado interno
                    if hasattr(component, 'update'):
                        component.update(self.state, current_time, dt)
                        # DEBUG: Ver si la fase se actualiza
                        if name == "macro_trajectory" and hasattr(component, 'phase'):
                            print(f"[DEBUG] Fase actualizada: {component.phase}")
                    ...
                    if hasattr(component, 'calculate_delta'):
                        ...
                        if delta and delta.position is not None:
                            # DEBUG: Ver si se generan deltas
                            if name == "macro_trajectory":
                                print(f"[DEBUG] Delta generado: {delta.position}")
                            # Asegurar que el delta tiene el source_id correcto
...
class MacroTrajectory(MotionComponent, MovementModeMixin):
    ...
    def __init__(self):
        ...
        self.speed = 1.0
        self.phase = 0.0
        self.last_position = np.zeros(3)  # Posición del centro del macro
        self.last_orientation = np.zeros(3)
        self.playback_controller = None  # Se asignará desde el engine
        self.current_frame = -1  # Para evitar múltiples cálculos por frame
        self._delta_cache = None  # Cache del delta para el frame actual
    ...
    def set_trajectory(self, 
        ...
        if position_func is not None:
            ...
            print(f"[DEBUG] MacroTrajectory.set_trajectory: función asignada, speed={speed}")
            # Obtener longitud de la trayectoria si está disponible
            if hasattr(position_func, 'length'):
                self.trajectory_length = position_func.length
                print(f"[DEBUG] Longitud detectada: {self.trajectory_length}")
            else:
    ...
    def update(self, state: MotionState, current_time: float, dt: float) -> MotionState:
        ...
        # DEBUG: Ver si tiene función de trayectoria
        if not self.trajectory_func:
            print(f"[DEBUG] MacroTrajectory.update: NO trajectory_func!")
            return state
            
        # Actualizar fase usando el playback controller si está disponible
        if self.playback_controller:
            # El controller maneja la actualización de fase según el modo
            self.phase = self.playback_controller.update(dt, self.phase)
        else:
            # Comportamiento por defecto (Fix mode)
            # Usar lógica de longitud si está disponible
            if hasattr(self.trajectory_func, 'length') and self.trajectory_func.length and self.trajectory_func.length > 0:
                # Velocidad real en m/s
                distance_to_travel = self.speed * dt
                phase_increment = distance_to_travel / self.trajectory_func.length
            else:
                # Fallback: comportamiento anterior
                phase_increment = self.speed * dt
            
            self.phase += phase_increment
        
        # Normalizar fase para mantener en rango [0, 1]
    ...
    def calculate_delta(self, state: 'MotionState', current_time: float, dt: float) -> Optional['MotionDelta']:
        """Calcular delta de movimiento para el CENTRO del macro"""
        if not self.enabled or not self.trajectory_func:
            return None
        
        # Sistema de frame tracking para evitar múltiples cálculos
        frame_id = int(current_time * 1000)  # Aproximar frame ID
        if self.current_frame == frame_id and self._delta_cache is not None:
            # Ya calculamos el delta para este frame, devolver copia
            return MotionDelta(
                position=self._delta_cache.position.copy() if self._delta_cache.position is not None else np.zeros(3),
                orientation=self._delta_cache.orientation.copy() if self._delta_cache.orientation is not None else np.zeros(3),
                aperture=self._delta_cache.aperture
            )
        
        # Nuevo frame, calcular delta
        self.current_frame = frame_id
        
        # Obtener posición objetivo de la trayectoria con la fase actual
        target_position = self.trajectory_func(self.phase)
        
        # Calcular delta desde la posición actual
        delta_position = target_position - self.last_position
        
        # DEBUG: Ver valores
        print(f"[DEBUG] target_position: {target_position}, last_position: {self.last_position}, delta: {delta_position}")
        
        # Actualizar última posición para próximo cálculo
        self.last_position = target_position.copy()
        
        # Manejar orientación si existe
        delta_orientation = np.zeros(3)
        if self.orientation_func:
            target_orientation = self.orientation_func(self.phase)
            delta_orientation = target_orientation - self.last_orientation
            self.last_orientation = target_orientation.copy()
        
        # Crear y cachear delta
        self._delta_cache = MotionDelta(
            position=delta_position.copy(),
            orientation=delta_orientation.copy(),
            aperture=0.0
        )
...
class MacroRotation(MotionComponent):
    ...
    def calculate_delta(self, state, current_time, dt):
        ...
        # Calcular ángulos de rotación para este frame (NO acumular)
        delta_angle_x = self.speed_x * dt_float
        delta_angle_y = self.speed_y * dt_float
        delta_angle_z = self.speed_z * dt_float
        ...
        # Precalcular senos y cosenos de los ángulos INCREMENTALES
        cx, sx = np.cos(delta_angle_x), np.sin(delta_angle_x)
        cy, sy = np.cos(delta_angle_y), np.sin(delta_angle_y)
        cz, sz = np.cos(delta_angle_z), np.sin(delta_angle_z)
...
class ManualMacroRotation(MotionComponent):
    ...
    def set_target_rotation(self, pitch: float = None, yaw: float = None, roll: float = None):
        ...
        # Reset flag cuando se establecen nuevos ángulos
        self.first_update = True
        self.enabled = True
    ...
    def calculate_delta(self, state: 'MotionState', current_time: float, dt: float) -> Optional['MotionDelta']:
        """Calcula el cambio instantáneo para la rotación manual"""
        if not self.enabled or not self.first_update:
            return None
            
        # MotionDelta está en este mismo archivo, no necesita importación
        delta = MotionDelta()
        
        current_position = np.array(state.position)
        relative_pos = current_position - self.center
        
        # Si está muy cerca del centro, no rotar
        ...
        # Calcular matriz de rotación 3D directamente con los ángulos objetivo
        ...
        # Aplicar rotación a la posición relativa
        rotated_pos = rotation_matrix @ relative_pos
        
        # Nueva posición absoluta
        new_position = self.center + rotated_pos
        
        # Calcular delta (movimiento completo en un solo frame)
    ...
    def _calculate_rotation_matrix(self, pitch: float, yaw: float, roll: float) -> np.ndarray:
        """
        Calcula matriz de rotación 3x3 desde ángulos de Euler
        
        Args:
            pitch: Rotación alrededor del eje X (radianes)
            yaw: Rotación alrededor del eje Y (radianes)
            roll: Rotación alrededor del eje Z (radianes)
    ...
    def update(self, current_time: float, dt: float, state: 'MotionState') -> 'MotionState':
        """Actualiza el estado aplicando la rotación manual instantánea"""
        if not self.enabled or not self.first_update:
            return state
...
class ConcentrationComponent(MotionComponent):
    ...
    def calculate_delta(self, state: MotionState, current_time: float, dt: float) -> MotionDelta:
        ...
        movement = direction * self.factor * dt
...
# === Agregar al final de motion_components.py ===
from enum import Enum
